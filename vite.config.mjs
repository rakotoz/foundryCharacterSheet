...
// the dev server.
const s_MODULE_ID = "revancharsheet";
    ...
    plugins: [
      ...
      viteStaticCopy({
        targets: [
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/assets")) + "/[!.]*", // 1️
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/assets`),
            ), // 2️
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/images")) + "/[!.]*", // 1️
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/images`),
            ), // 2️
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/icons")) + "/[!.]*", // 1️
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/icons`),
            ), // 2️
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/templates")) +
              "/[!.]*", // 1️
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/templates`),
            ), // 2️
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/lang")) + "/[!.]*",
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/lang`),
            ),
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/languages")) +
              "/[!.]*",
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/languages`),
            ),
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/styles")) +
              "/**/*.css",
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/styles`),
            ),
          },
          {
            src:
              normalizePath(path.resolve(__dirname, "./src/packs")) + "/[!.]*",
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/packs`),
            ),
          },
          {
            src: normalizePath(path.resolve(__dirname, "./src/module.json")),
            dest: normalizePath(
              path.resolve(__dirname, `./dist/${s_MODULE_ID}/`),
            ),
          },
        ],
      }),
